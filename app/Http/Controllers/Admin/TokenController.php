...
class TokenController extends Controller
{
    ...
    { 
        $tokens = TokenSetting::select('token_setting.*', 'college.name as college', 'counter.name as counter', 'user.firstname', 'user.lastname')
                ->leftJoin('college', 'token_setting.college_id', '=', 'college.id')
        ...
        $collegeList = College::where('status',1)->pluck('name','id');
        ...
        return view('backend.admin.token.setting', compact('tokens','countertList','collegeList','userList')); 
    }
    ...
    {   
        ...
        if ($validator->fails()) {
            ...
            $check = TokenSetting::where('college_id',$request->college_id)
    ...
    public function tokenAutoView()
    {
        ...
        return view('backend.admin.token.auto', compact('display', 'collegeList', 'keyList'));
    }    
    ...
    {  
        ...
        return view('backend.admin.token.manual', compact('display', 'counters', 'colleges','officers' ));
    }  
    ...
    {  
        ...
        return view('backend.admin.token.current', compact('counters', 'colleges', 'officers', 'tokens'));
    } 
    ...
    {  
        ...
        return view('backend.admin.token.report', compact('counters', 'colleges', 'officers'));
    }  
    ...
    public function reportData(Request $request)
    {
        ...
        { 
            $tokensProccess = Token::where(function($query)  use($search) {
                    ...
                    if (!empty($search['college'])) {
                        $query->where('college_id', '=', $search['college']);
                    }
        ...
        if(!empty($tokens))
        {
            ...
            {  
                ...
                $data[] = [
                    ...
                    'college'    => (!empty($token->college)?$token->college->name:null),
    ...
    public function recall($id = null)
    {
        ...
        $token = DB::table('token AS t')
            ...
            ->leftJoin('college AS d', 'd.id', '=', 't.college_id')
