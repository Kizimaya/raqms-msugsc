        ...
        <div class="col-sm-12" id="screen-content">
            @if($display->sms_alert || $display->show_note)
                ...
                <div class="p-1 m-1 btn btn-primary capitalize text-center">
                    <button 
                        ...
                        style="min-width: 15vw;white-space: pre-wrap;box-shadow:0px 0px 0px 2px#<?= substr(dechex(crc32($college->name)), 0, 6); ?>" 
                        data-toggle="modal" 
                        data-target="#tokenModal"
                        data-college-id="{{ $college->college_id }}"
                        data-counter-id="{{ $college->counter_id }}"
                        data-user-id="{{ $college->user_id }}"
                        >
                            <h5>{{ $college->name }}</h5>
                            <h6>{{ $college->officer }}</h6>
                    </button>  
                </div>
                @endforeach  
            ...
            @else
                ...
                @foreach ($collegeList as $college )
                  ...
                  <input type="hidden" name="counter_id" value="{{ $college->counter_id }}">
                  <input type="hidden" name="user_id" value="{{ $college->user_id }}">
                  <button 
                    type="submit" 
                    class="p-1 m-1 btn btn-primary capitalize text-center"
                    style="min-width: 15vw;white-space: pre-wrap;box-shadow:0px 0px 0px 2px#<?= substr(dechex(crc32($college->name)), 0, 6); ?>" 
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <p><strong class="label label-warning"> Note 2 </strong> &nbsp; To display a college on the auto token setting page, you need to set up it in Auto Token Setting page. 
        </p>
        <p><strong class="label label-warning"> Note 3 </strong> &nbsp; 
            ...
            To create a token for a college, press on the key which you have denoted in the <strong>key for keyboard mode</strong> field in the add college page. 
...
$(document).ready(function(){
    $('.modal').on('show.bs.modal', function (event) {
        ...
        $('input[name=college_id]').val(button.data('college-id'));
        ...
        $.ajax({
            ...
            success: function(data)
            {
                if (data.status)
                {
                    ...
                    content += "<li><strong>{{ trans('app.college') }} </strong>"+data.token.college+"</li>";
            ...
            $.each(JSON.parse(keyList), function (id, obj) {
                if (obj.key == key) {
                    ...
                    @if($display->sms_alert || $display->show_note)
                        ...
                        modal.modal('show');
                        modal.find('input[name=college_id]').val(obj.college_id);
                    ...
                    @else
                        ...
                        formData.append("college_id", obj.college_id);
